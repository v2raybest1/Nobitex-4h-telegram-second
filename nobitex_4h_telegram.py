...
# ============================================================
# Nobitex 4H Telegram Scanner - 22 Symbols
...
def http_get_json(path, params):
    ...
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "Mozilla/5.0 GitHubActions NobitexScanner/4.3-25Symbols",
...
def fetch_4h_history(symbol, server_utc):
    to_ts = int(server_utc.timestamp())

    # ask for a slightly wider window than strictly needed
    from_ts = to_ts - ((HISTORY_4H_CANDLES + 10) * FOUR_HOURS)
    ...
    for i in range(lengths[0]):
        try:
            ...
            rows.append({
                "ts": ts,
                "start": start_teh,
                "end": start_teh + timedelta(hours=4),
...
def validate_and_select_complete(rows, server_utc):
    if len(rows) < 120:
        return False, "کندل کافی نیست", None

    # Validate direct Nobitex 4H candle alignment against Tehran boundaries.
    bad = []
    for r in rows[-150:]:
        st = r["start"]
        if (
            st.hour not in (0, 4, 8, 12, 16, 20)
            or st.minute != 0
            or st.second != 0
        ):
            bad.append(st)
    ...
    if bad:
        ...
        return (
            False,
            "مرزبندی کندل 4H نوبیتکس با تهران منطبق نیست "
    ...
    for prev, cur in zip(recent, recent[1:]):
        if cur["ts"] - prev["ts"] != FOUR_HOURS:
            return False, "توالی کندل‌های 4H ناقص است", None
...
def analyze_symbol(symbol, server_utc):
    try:
        rows, response_server_utc = fetch_4h_history(symbol, server_utc)
...
def build_report_message(scan):
    ...
    if interval is not None:
        start, end = interval
        lines.append(
            f"کندل: {start.strftime('%H:%M')} → {end.strftime('%H:%M')} تهران"
        )
    else:
    ...
    if scan.get("inconsistent"):
        lines.append("⚠️ آخرین کندل کامل نمادهای سالم یکسان نیست.")
    else:
        ...
        if alerts:
            ...
            lines.append("✅ هیچ سیگنال قطعی یا نزدیکی وجود ندارد.")
...
def split_message(text, limit=3800):
    ...
    continuation = "📊 ادامه گزارش Nobitex 4H\n"
    chunks = []
    current = ""
...
def send_telegram(text, token, chat_id, retries=3):
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "Accept": "application/json",
            "User-Agent": "Nobitex4HTelegram/1.0",
        },
        method="POST",
    )
